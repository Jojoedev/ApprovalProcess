...
namespace ApprovalProcess.Pages.Logics.RequestLogic
{
    ...
    public class LastApprovalUpdateModel : PageModel
    {
        private readonly ApplicationDbContext _context;
        public LastApprovalUpdateModel(ApplicationDbContext context)
        {
            _context = context; 
        }
        ...
        public ActionResult OnPost(Request request)
        {
            if (ModelState.IsValid)
            {
                ...
                _context.Entry(request).Property(x => x.FinanceDirector).IsModified = false;
                _context.Entry(request).Property(x => x.ManagingDirector).IsModified = true;
